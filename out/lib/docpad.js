  ...
  DocPad = (function(_super) {
    ...
    */
    ...
    DocPad.prototype.getCollection = function(value) {
      var collection, _i, _j, _len, _len1, _ref1, _ref2;
      if (value) {
    ...
    DocPad.prototype.eachCollection = function(fn) {
      var collection, index, _i, _len, _ref1;
      fn(this.getDatabase(), 'database');
      _ref1 = this.collections;
      for (index = _i = 0, _len = _ref1.length; _i < _len; index = ++_i) {
        collection = _ref1[index];
    ...
    DocPad.prototype.getLocale = function() {
      ...
      if ((this.locale != null) === false) {
        ...
        codes = _.uniq(['en', safeps.getLanguageCode(config.localeCode), safeps.getLanguageCode(safeps.getLocaleCode()), safeps.getLocaleCode(config.localeCode), safeps.getLocaleCode(safeps.getLocaleCode())]);
        locales = (function() {
          var _i, _len, _results;
          _results = [];
          for (_i = 0, _len = codes.length; _i < _len; _i++) {
            code = codes[_i];
            _results.push(this.loadLocale(code));
          }
          return _results;
        }).call(this);
    ...
    DocPad.prototype.loadLocale = function(code) {
      ...
      localePath = pathUtil.join(this.localePath, "" + code + ".cson");
    ...
    DocPad.prototype.getHostname = function() {
      var _ref1, _ref2;
      return (_ref1 = (_ref2 = this.getConfig().hostname) != null ? _ref2 : process.env.HOSTNAME) != null ? _ref1 : null;
    };
    ...
    function DocPad(instanceConfig, next) {
      ...
      this.actionRunnerInstance = new TaskGroup('action runner').run().on('complete', function(err) {
        if (err) {
          return docpad.error(err);
        }
      });
      this.errorRunnerInstance = new TaskGroup('error runner').run().on('complete', function(err) {
        var locale, _ref2;
      ...
      this.trackRunnerInstance = new TaskGroup('track runner').run().on('complete', function(err) {
        var locale, _ref2;
        if (err && docpad.getDebugging()) {
          locale = docpad.getLocale();
      ...
      this.database = new FilesCollection(null, {
        ...
        var outPath, updatedModels, url, _i, _len, _ref2;
        ...
        if (outPath) {
          updatedModels = _this.database.findAll({
            outPath: outPath
          });
        ...
        var url, _i, _j, _len, _len1, _ref2, _ref3;
        if (model.get('write') === false) {
          return;
        }
        ...
        var existingModel, existingModelId, existingModelPath, message, modelPath, outPath, previousModelId, previousModels, previousOutPath, _base;
        if (model.get('write') === false) {
          return;
        }
        ...
        if ((outPath = model.get('outPath'))) {
          ...
          if (existingModelId !== model.id) {
            ...
            if (existingModel) {
              ...
              _this.filesByOutPath[outPath] = model.id;
            }
          }
        }
        return true;
    ...
    DocPad.prototype.ready = function(opts, next) {
      ...
      tasks = new TaskGroup('ready tasks', {
        next: function(err) {
          if (err) {
            return docpad.error(err);
          }
    ...
    DocPad.prototype.setConfig = function(instanceConfig, next) {
      ...
      postTasks = new TaskGroup('setConfig post tasks', {
        next: function(err) {
    ...
    DocPad.prototype.load = function(instanceConfig, next) {
      ...
      this.config = {};
      this.setInstanceConfig(instanceConfig);
      preTasks = new TaskGroup('load tasks', {
        next: function(err) {
          if (err) {
            return next(err);
          }
      ...
      preTasks.addTask('normalize the userConfigPath', function(complete) {
      ...
      preTasks.addTask("load the user's configuration", function(complete) {
        var configPath;
      ...
      preTasks.addTask("load the anonymous user's configuration", function(complete) {
        ...
        return require('getmac').getMac(function(err, macAddress) {
          ...
          if (macAddressHash) {
            if ((_base = _this.userConfig).name == null) {
              _base.name = "MAC " + macAddressHash;
            }
            if ((_base1 = _this.userConfig).username == null) {
      ...
      preTasks.addTask("load the website's package data", function(complete) {
      ...
      preTasks.addTask("read the .env file if it exists", function(complete) {
        var configPath, rootPath;
      ...
      preTasks.addTask("load the website's configuration", function(complete) {
    ...
    DocPad.prototype.updateUserConfig = function(data, next) {
      ...
      CSON.stringify(this.userConfig, function(err, userConfigString) {
        ...
        return safefs.writeFile(userConfigPath, userConfigString, 'utf-8', function(err) {
          return typeof next === "function" ? next(err) : void 0;
        });
      });
      return this;
    ...
    DocPad.prototype.getConfigPath = function(opts, next) {
      ...
      tasks = new TaskGroup('getConfigPath tasks', {
        next: function(err) {
          return next(err, result);
        }
      });
    ...
    DocPad.prototype.extendCollections = function(next) {
      ...
      tasks = new TaskGroup("extendCollections tasks", {
        concurrency: 0,
        next: function(err) {
          if (err) {
            docpad.error(err);
    ...
    DocPad.prototype.resetCollections = function(opts, next) {
      var database, docpad, meta, scripts, styles, _ref1;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      docpad = this;
    ...
    DocPad.prototype.notify = function(message, options) {
      var docpad;
      if (options == null) {
        options = {};
      }
    ...
    DocPad.prototype.subscribe = function(next) {
      var config, data, err, _ref1;
      config = this.getConfig();
      if (config.offline === false) {
        if ((_ref1 = this.userConfig) != null ? _ref1.email : void 0) {
          data = {};
    ...
    DocPad.prototype.track = function(name, things, next) {
      ...
      if (config.offline === false && config.reportStatistics) {
        ...
        if ((_ref1 = this.websitePackageConfig) != null ? _ref1.name : void 0) {
    ...
    DocPad.prototype.createModels = function(models, opts) {
      ...
      for (_i = 0, _len = models.length; _i < _len; _i++) {
        model = models[_i];
        _results.push(this.createModel(model, opts));
      }
      return _results;
    };

    DocPad.prototype.createModel = function(attrs, opts) {
      var config, database, dirPath, docpad, fileFullPath, model, result, _i, _j, _len, _len1, _ref1, _ref2;
      if (attrs == null) {
        attrs = {};
      }
      if (opts == null) {
    ...
    DocPad.prototype.loadPlugins = function(next) {
      ...
      tasks = new TaskGroup("loadPlugins tasks", {
        concurrency: 0,
        next: function(err) {
          docpad.slowPlugins = {};
          snore.clear();
    ...
    DocPad.prototype.contextualizeFiles = function(opts, next) {
      var collection, config, docpad, locale, slowFilesObject, slowFilesTimer, templateData, _ref1, _ref2;
      if (opts == null) {
        opts = {};
      }
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      ...
      if ((_ref2 = opts.progress) != null) {
        _ref2.step("contextualizeFiles (preparing)").total(1).setTick(0);
      }
      ...
      docpad.emitSerial('contextualizeBefore', {
        ...
        tasks = new TaskGroup("contextualizeFiles tasks", {
          concurrency: 0,
          next: function(err) {
            var _ref3;
            ...
            if ((_ref3 = opts.progress) != null) {
              _ref3.step("contextualizeFiles (postparing)").total(1).setTick(0);
            }
        ...
        if ((_ref3 = opts.progress) != null) {
          _ref3.step('contextualizeFiles').total(collection.length).setTick(0);
        }
    ...
    DocPad.prototype.renderFiles = function(opts, next) {
      var collection, config, docpad, locale, renderCollection, renderFile, renderPasses, slowFilesObject, slowFilesTimer, templateData, _ref1, _ref2,
        _this = this;
      if (opts == null) {
        opts = {};
      }
      ...
      slowFilesTimer = null;
      if ((_ref2 = opts.progress) != null) {
        _ref2.step("renderFiles (preparing)").total(1).setTick(0);
      }
      ...
      renderCollection = function(collectionToRender, _arg, next) {
        ...
        return docpad.emitSerial('renderCollectionBefore', {
          ...
          subTasks = new TaskGroup("renderCollection: " + collectionToRender.options.name, {
            concurrency: 0,
            next: function(err) {
              if (err) {
                return next(err);
          ...
          if ((_ref3 = opts.progress) != null) {
            _ref3.step("renderFiles (pass " + renderPass + ")").total(collectionToRender.length).setTick(0);
          }
      ...
      docpad.emitSerial('renderBefore', {
        ...
        tasks = new TaskGroup("renderCollection: renderBefore tasks", {
          next: function(err) {
            var _ref3;
            clearInterval(slowFilesTimer);
            slowFilesTimer = null;
            ...
            if ((_ref3 = opts.progress) != null) {
              _ref3.step("renderFiles (postparing)").total(1).setTick(0);
            }
    ...
    DocPad.prototype.writeFiles = function(opts, next) {
      ...
      slowFilesTimer = null;
      if ((_ref2 = opts.progress) != null) {
        _ref2.step("writeFiles (preparing)").total(1).setTick(0);
      }
      ...
      docpad.emitSerial('writeBefore', {
        ...
        tasks = new TaskGroup("writeFiles tasks", {
          concurrency: 0,
          next: function(err) {
            var _ref3;
            clearInterval(slowFilesTimer);
            ...
            if ((_ref3 = opts.progress) != null) {
              _ref3.step("writeFiles (postparing)").total(1).setTick(0);
            }
        ...
        if ((_ref3 = opts.progress) != null) {
          _ref3.step('writeFiles').total(collection.length).setTick(0);
        }
        collection.forEach(function(file, index) {
          var filePath;
          filePath = file.getFilePath();
          return tasks.addTask("writing the file: " + filePath, function(complete) {
            var fileTasks;
            slowFilesObject[file.id] = file.get('relativePath');
            fileTasks = new TaskGroup("tasks for file write: " + filePath, {
              concurrency: 0,
              next: function(err) {
                var _ref4;
                delete slowFilesObject[file.id];
    ...
    DocPad.prototype.generate = function(opts, next) {
      var addGroup, addTask, config, database, docpad, lastGenerateStarted, locale, tasks, _ref1, _ref2, _ref3;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      docpad = this;
      config = docpad.getConfig();
      locale = docpad.getLocale();
      ...
      tasks = new TaskGroup("generate tasks").on('item.run', function(item) {
        var totals, _ref3;
        totals = tasks.getTotals();
        return (_ref3 = opts.progress) != null ? _ref3.step("generate: " + (item.getConfig().name)).total(totals.total).setTick(totals.completed) : void 0;
      }).once('complete', function(err) {
        var howMany, seconds, _ref3;
        ...
        howMany = "" + (((_ref3 = opts.collection) != null ? _ref3.length : void 0) || 0) + "/" + database.length;
      ...
      addGroup('fetch data to render', function(addGroup, addTask) {
        ...
        if (opts.populate === true) {
          ...
          if (opts.initial === true && ((_ref3 = opts.cache) === true || _ref3 === 'read')) {
          ...
          return collection.forEach(function(file) {
            ...
            if (file.get('isLayout') === true) {
              ...
              docpad.log('debug', 'Layout children to generate at', lastGenerateStarted, '\n', (function() {
                var _i, _len, _ref3, _results;
                _ref3 = layoutChildrenCollection.models;
                _results = [];
                for (_i = 0, _len = _ref3.length; _i < _len; _i++) {
                  model = _ref3[_i];
                  _results.push({
    ...
    DocPad.prototype.renderText = function(text, opts, next) {
      ...
      document = this.createDocument(attributes);
      document.action('normalize contextualize render', opts, function(err) {
        var result;
        result = document.getOutContent();
    ...
    DocPad.prototype.watch = function(opts, next) {
      ...
      restartWatchers = function(next) {
        ...
        docpad.destroyWatchers();
        tasks = new TaskGroup("watch tasks", {
          concurrency: 0,
          next: next
        });
    ...
    DocPad.prototype.initInstall = function(opts, next) {
      ...
      docpad = this;
      config = this.getConfig();
      tasks = new TaskGroup("initInstall tasks", {
        concurrency: 0,
        next: next
      });
    ...
    DocPad.prototype.uninstall = function(opts, next) {
      ...
      docpad = this;
      config = this.getConfig();
      tasks = new TaskGroup("uninstall tasks", {
        next: next
      });
      if (opts.plugin) {
    ...
    DocPad.prototype.install = function(opts, next) {
      ...
      docpad = this;
      config = this.getConfig();
      tasks = new TaskGroup("install tasks", {
        next: next
      });
    ...
    DocPad.prototype.update = function(opts, next) {
      ...
      docpad = this;
      config = this.getConfig();
      tasks = new TaskGroup("update tasks", {
        next: next
      });
    ...
    DocPad.prototype.clean = function(opts, next) {
      ...
      tasks = new TaskGroup("clean tasks", {
        concurrency: 0
      }, {
        next: function(err) {
          if (err) {
      ...
      tasks.addTask('delete out path', function(complete) {
        ...
        return balUtil.rmdirDeep(config.outPath, complete);
      });
    ...
    DocPad.prototype.initSkeleton = function(skeletonModel, opts, next) {
      ...
      tasks = new TaskGroup("initSkeleton tasks", {
        next: next
      });
    ...
    DocPad.prototype.serveDocument = function(opts, next) {
      var collection, config, content, contentType, ctime, docpad, document, dynamic, encoding, err, etag, mtime, req, res, stat, templateData, _ref1;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      ...
      if (res.setHeaderIfMissing == null) {
        res.setHeaderIfMissing = function(name, value) {
          if ((res.getHeader(name) != null) === false) {
      ...
      res.setHeaderIfMissing('Content-Type', contentType + (encoding !== 'binary' ? "; charset=" + encoding : ''));
      if (config.maxAge) {
    ...
    DocPad.prototype.server = function(opts, next) {
      var config, docpad, express, hostname, http, locale, port, servers, tasks, _ref1, _ref2, _ref3, _ref4;
      _ref1 = extractOptsAndCallback(opts, next), opts = _ref1[0], next = _ref1[1];
      docpad = this;
      config = this.config;
      locale = this.getLocale();
      ...
      tasks = new TaskGroup("server tasks", {
        next: next
      });
