...
(function() {
  var Backbone, FileModel, Model, TaskGroup, docpadUtil, encodingUtil, extendr, extractOptsAndCallback, isTextOrBinary, jschardet, mime, pathUtil, safefs, typeChecker, _ref, _ref1,
  ...
  FileModel = (function(_super) {
    ...
    FileModel.prototype.initialize = function(attrs, opts) {
      ...
      if (this.id == null) {
        this.id = (_base4 = this.attributes).id != null ? (_base4 = this.attributes).id : _base4.id = this.cid;
      }
      this.setOptions(opts);
      this.actionRunnerInstance = new TaskGroup("file action runner").run().on('complete', function(err) {
        if (err) {
    ...
    FileModel.prototype.load = function(opts, next) {
      var file, filePath, fullPath, tasks, _ref2;
      if (opts == null) {
        opts = {};
      }
    ...
    FileModel.prototype.parse = function(opts, next) {
      var buffer, changes, content, d, encoding, err, isText, relativePath, source, _ref2, _ref3,
        _this = this;
      if (opts == null) {
        opts = {};
      }
      ...
      if (buffer && (encoding != null) === false || opts.reencode === true) {
        ...
        if (isText === true) {
          if (this.detectEncoding) {
            ...
            if (encoding == null) {
              encoding = (_ref3 = jschardet.detect(buffer)) != null ? _ref3.encoding : void 0;
            }
          }
          ...
          if (docpadUtil.isStandardEncoding(encoding) === false) {
            ...
            if (encodingUtil != null) {
              ...
              d.run(function() {
                return buffer = encodingUtil.convert(buffer, 'utf-8', encoding);
              });
            } else {
              this.log('warn', "Encoding utilities did not load, therefore we cannot convert the encoding " + encoding + " to UTF-8 on " + relativePath);
            }
          }
      ...
      if (encoding === 'binary') {
        ...
        if ((encoding != null) === false) {
          encoding = changes.encoding = 'utf-8';
        }
        source = (buffer != null ? buffer.toString('utf-8') : void 0) || '';
        content = source;
        changes.content = content;
        changes.source = source;
      }
    ...
    FileModel.prototype.write = function(opts, next) {
      ...
      opts.encoding || (opts.encoding = file.get('encoding') || 'utf-8');
      ...
      if ((_ref3 = opts.encoding.toLowerCase()) !== 'ascii' && _ref3 !== 'utf8' && _ref3 !== 'utf-8' && _ref3 !== 'binary') {
        ...
        if (encodingUtil != null) {
          ...
          try {
            opts.content = encodingUtil.convert(opts.content, opts.encoding, 'utf-8');
          } catch (_error) {
            err = _error;
            this.log('warn', "Encoding conversion failed, therefore we cannot convert the encoding UTF-8 to " + opts.encoding + " on " + opts.path);
          }
        } else {
          this.log('warn', "Encoding utilities did not load, therefore we cannot convert the encoding UTF-8 to " + opts.encoding + " on " + opts.path);
        }
      }
